...
volatile uint16_t nextBinary;  // index for the next value to display
volatile uint8_t isOneSec = 0; // boolean variable which stores the value 'is the delay one?'
...
int main(void)
{
  ...
  nextBinary = 0; // initialise to start with first pattern
  ...
  while (1)
  {
    ...
    //Wait for the button PA0 to be pressed
    while ((GPIOA->IDR & GPIO_IDR_0) != 0)
      ;
    // Check if the delay is 1, if 1 make it half 1 (divide by 2) else make it 1 (multiply half by 2)
    // then change the variable of whether its now 1 or not
    if (isOneSec)
    {
      TIM16->ARR <<= 1;
      isOneSec = 0;
...
void TIM16_IRQHandler(void)
{
  ...
  uint8_t binaryPattern = read_from_address(nextBinary);

  if (binaryPattern != patterns[nextBinary])
  {
    binaryPattern = 0x01;
  }
  GPIOB->ODR = binaryPattern;
  nextBinary++;
